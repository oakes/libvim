    ...
    int merge)
{
  if (buf->b_recorded_changes != NULL && xtra != 0)
  {
         ...
         li = li->li_next)
    {
      ...
      if (prev_lnum >= lnum || prev_lnum > lnume || (prev_lnume >= lnum && xtra != 0))
      {
        if (li->li_next == NULL && lnum == prev_lnum && xtra == 0 && col + 1 == (colnr_T)dict_get_number(li->li_tv.vval.v_dict, (char_u *)"col"))
        {
          if (merge)
          {
            dictitem_T *di;
 ...
 */
void f_listener_remove(typval_T *argvars, typval_T *rettv UNUSED)
{
  listener_T *lnr;
  listener_T *next;
